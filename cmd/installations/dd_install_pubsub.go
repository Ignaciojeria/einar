...
func DDInstallPubSub(project string) error {
	...
	for _, installCommand := range config.InstallationCommands {
		if installCommand.Name == "dd-pubsub" {
	...
	if len(pubsubLibs) == 0 {
		err = fmt.Errorf("dd-pubsub libraries not found in .einar.cli.json")
		fmt.Println(err)
		return err
	}
	...
	sourceDir := filepath.Join(filepath.Dir(binaryPath), "app/shared/archetype/dd_pubsub")
